...
\begin{enumerate}[label=\alph*)]
    ...
    \item \textbf{Dependência de Serviços de Terceiros}: A utilização da hospedagem introduz riscos relacionados a custos e disponibilidade.
\end{enumerate}
\end{spacing}
...
\begin{spacing}{1.5}
Na Figura \ref{fig:C4-Contexto} é apresentado o Diagrama de Contexto, ilustrando as interações do sistema FazendaPro com atores externos, como o usuário (produtor).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.2\textwidth]{images/c1-new.drawio.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/c2-new.drawio.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/c3-new.drawio.png}
...
\begin{spacing}{1.5}
A aplicação web será desenvolvida com React, enquanto o API Server, hospedado em containers via Dokploy, utilizará NestJS, Go com Memcached para caching em memória, garantindo desempenho na gestão de dados pecuários.
\end{spacing}

\subsubsection{Armazenamento Persistente de Dados}
\begin{spacing}{1.5}
O armazenamento utiliza PostgreSQL gerenciado via Dokploy. A aplicação web realiza requisições HTTP (REST) ao API Server, que consulta o banco de dados PostgreSQL via conexão SQL e utiliza Memcached para caching, otimizando o acesso a dados de rebanhos e produção leiteira.
\end{spacing}
...
\item \textbf{PostgreSQL} – Sistema de gerenciamento de banco de dados relacional avançado, gerenciado via Dokploy.
...
\item \textbf{Dokploy} – Plataforma de deploy self-hosted para aplicações containerizadas.
...
\item \textbf{Dokploy}: Plataforma de deploy self-hosted que oferece controle total sobre a infraestrutura, facilitando o gerenciamento de containers e bancos de dados, com suporte a CI/CD e monitoramento.
\end{enumerate}
\end{spacing}
...
\begin{enumerate}[label=\alph*)]
\item \textbf{Servidor VPS}: \$20-40/mês (2-4 vCPU, 4-8GB RAM, dependendo do provedor)
\item \textbf{PostgreSQL}: Incluído no servidor (self-hosted via Dokploy)
\item \textbf{Memcached}: Incluído no servidor (self-hosted via Dokploy)
\item \textbf{Total estimado}: \$20-40/mês para até 100 usuários (dependendo do provedor de VPS escolhido)
\end{enumerate}
...
\begin{enumerate}
    ...
    \item DOKPLOY. \textit{Dokploy Documentation}. Disponível em: \url{https://dokploy.com}. Acesso em: 17 jun. 2025.
    ...
    Disponível em: \url{https://wwebjs.dev}. Acesso em: 17 jun. 2025.
\end{enumerate}
